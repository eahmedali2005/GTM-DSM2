...
module advection
    ...
    subroutine update_conservative(mass,                &
        ...
        ! old time step
        do ivar=1,nvar
            mass(:,ivar) = mass_prev(:,ivar) - dtbydx*div_flux(:,ivar) + dt/(nvar-1)*source_prev(:,ivar)*area_prev + dt*explicit_diffuse_op(:,ivar)
        end do
       
        do j=1,5  ! 5 here is a fixed number that represents the number of iterations to correct/update the
                    !mass after considering the mass-fraction (this is the function of this do loop)
         
           do i=1,ncell  
              mass_tot(i)=0
              
              do ivar=1,nvar  !the function of this loop is to obtain the total suspended solids mass at each cell
                 mass_tot(i)=mass_tot(i)+mass(i,ivar)
              end do 
              do ivar=1,nvar
                 mass(:,ivar) = mass_prev(:,ivar) - dtbydx*div_flux(:,ivar) + dt*mass(i,ivar)/mass_tot(i)*source_prev(:,ivar)*area_prev + dt*explicit_diffuse_op(:,ivar)
              end do  
           end do   
        ...
        do ivar = 1,nvar
            mass(:,ivar) =  mass_prev(:,ivar)                     &
                          ...
                          + dt/(nvar-1)*half*source_prev(:,ivar)*area_prev &
        ...
        do j=1,5  ! 5 here is a fixed number that represents the number of iterations to correct/update the
           ...
           do i=1,ncell  
              ...
              do ivar=1,nvar
                 mass(:,ivar) =  mass_prev(:,ivar)                     &
                          - dtbydx*div_flux(:,ivar)               &
                          + dt*mass(i,ivar)/mass_tot(i)*half*source_prev(:,ivar)*area_prev &
                          + dt*half*source(:,ivar)*area
              end do  
           end do   
